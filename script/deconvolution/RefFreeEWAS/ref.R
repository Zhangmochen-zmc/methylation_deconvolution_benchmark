...
# We will filter out 9999 probes with significant differences; an error will occur if the number exceeds 10000.
variance <- apply(averaged_df, 1, var, na.rm = TRUE)
most_variant_9999 <- averaged_df[names(variance[order(variance, decreasing = TRUE)][1:9999]), ]
