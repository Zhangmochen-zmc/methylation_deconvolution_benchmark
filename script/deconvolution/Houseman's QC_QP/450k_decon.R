...
select_signature <- function(beta_ref, cell_ref, cellTypes,
  ...
  # both: n_each of the highly methylated end and the lowly methylated end.
...
houseman_qp <- function(Y, W, sum_to_one = TRUE) {
  ...
  stopifnot(all(rownames(Y) == rownames(W)))
  K <- ncol(W)
  nS <- ncol(Y)
  ct <- colnames(W)
...
# B. Prepare reference data (load once)
cat(">>> loading Reference RDS...\n")
cell_ref_global <- readRDS(file.path(ref_dir, "cell_ref_own_450k.rds"))
beta_ref_global <- readRDS(file.path(ref_dir, "beta_ref_own_450k.rds"))
cellTypes <- unique(cell_ref_global)
cat("    Reference Loading complete。\n\n")

# C. Batch Loop Processing

csv_files <- list.files(input_folder, pattern = "\\.csv$", full.names = TRUE)
benchmark_log <- data.frame()

if (length(csv_files) == 0) stop("CSV file not found！")
cat(paste0(">>> Start processing ", length(csv_files), " files...\n\n"))
...
for (i in seq_along(csv_files)) {
  ...
  monitor_res <- peakRAM({
    ...
    # 2. Find the intersection of your beta with CpG
    # -------------------------------------------------
    ...
    if (length(common_cpg) < 200) {
      ...
      # 4. Computational Reference Center
      # -------------------------------------------------
      ...
      if (is.null(feat)) {
        ...
        # 6. Construct W and Y
        W <- ref_centroids[feat, , drop = FALSE]  # CpG x cellTypes
        Y <- beta_use[feat, , drop = FALSE]       # CpG x samples
        ...
        # 8. save
        # -------------------------------------------------
        output_txt_path <- file.path(output_folder, paste0(file_name, "_Houseman_QP_result.txt"))
        write.table(P, file = output_txt_path, sep = "\t", col.names = NA, quote = FALSE)
      }
    }
  })
  # Monitoring Ended
